...
class AuthManager {
    ...
    func signUp(withName name: String, withEmail email: String, withUsername username: String, withPassword password: String, completion: ((Result<User, Error>)->Void)?) {
        ...
        DatabaseRequest.shared.setUser(u) { () }
        self?.linkUserCopy(withuid: authResult.user.uid, completion: completion)
        }
    }
    
    private func linkUserCopy(withuid uid: String,
